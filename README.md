...
##Comment down lines 58 and 172 in settings.py for local development
...
EXPOSE 8000
CMD ["gunicorn", "claims_interface.wsgi:application"]
```
...
### Common Issues
1. **Database Connection**: Check database settings and permissions
2. **Static Files**: Run `python manage.py collectstatic`
3. **HTMX Issues**: Ensure JavaScript is enabled
4. **Authentication**: Verify user credentials and permissions
...
- **v1.1.0**: Added HTMX integration
- **v1.2.0**: Enhanced analytics dashboard
- **v1.3.0**: Improved data ingestion capabilities

---

**Built with ❤️ using Django, Alpine.js, and HTMX**
